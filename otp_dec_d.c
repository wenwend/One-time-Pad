//This program performs as server to decrppt.
...
void decrypt(char ciphertext[], char key[]){
	...
	for(i=0;ciphertext[i]!='\n';i++){
		//char ch=plaintext[i];
		n=charToInt(ciphertext[i]) - charToInt(key[i]);
		if(n<0) n+=27;
		//printf("plaintext[i] = %c, key[i] = %c\n", plaintext[i], key[i]);
		ciphertext[i]=intToChar(n);
	}
	ciphertext[i]='\0';
...
int main(int argc, char *argv[])
{
	...
	while (1) {
		...
		if(pid==0){
			...
			char ciphertext[128000];
			memset(ciphertext,'\0',sizeof(ciphertext));
			strncpy(ciphertext,buffer,keyPos-buffer);
			//printf("encrypt SERVER: message before encrypt %s \n", plaintext);
			decrypt(ciphertext,keyPos);
			//printf("encrypt SERVER: message after encrypt %s\n", plaintext);
			int r;
			r=send(establishedConnectionFD,ciphertext,sizeof(ciphertext),0);
			if(r<strlen(ciphertext)){
				fprintf(stderr,"decrypt server writing too short\n");
				exit(2);
			}
