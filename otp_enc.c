//This client side request to connect opt_enc_d server and request an encrypt.
//exit 1 when bad character in plaintest, exit 2 when trying to connect to wrong server such as: decrypt.
...
int main(int argc, char *argv[])
{
	...
	char auth[]="enc_ask_right";
	memset(buffer,'\0',sizeof(buffer));
	//verify the right server!
	send(socketFD,auth,sizeof(auth),0);
	recv(socketFD,buffer,sizeof(buffer),0);
	if(strcmp(buffer,"enc_rep_right")!=0){fprintf(stderr, "CLIENT: Connectiong to wrong server\n"); exit(2);}
